...
app.config['UPLOAD_FOLDER'] = os.path.join('static', 'uploads', 'profile')
...
def checkout_confirm():
    ...
    # Get logged-in users from session
    users = load_users()
    logged_in_user = next((u for u in users if u["id"] == session["user_id"]), None)
    ...
    # Compare billing email with logged-in users's email
...
def upload_profile():
    ...
    if file and allowed_file(file.filename):
        ...
        # Update users record
        users = load_users()
        user = users[-1]  # for now, last registered users
...
class User(db.Model):
    ...
    contact = db.Column(db.String(128), unique=True)
...
def delete_user(user_id):
    ...
    if request.method == "POST":
        # Delete profile image if it's not the default image
